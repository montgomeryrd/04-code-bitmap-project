...
transformation.red = function(bmp) {
    for(let i = 0 ; i < bmp.colors.length/4; i++) {
        bmp.colors[i * 4 + 2] = 255;
    }
    return bmp;
};

transformation.green = function(bmp) {
    for(let i = 0 ; i < bmp.colors.length/4 ; i++) {
        bmp.colors[i * 4 + 1] = 255;
    }
    return bmp;
};

transformation.blue = function(bmp) {
    for(let i = 0 ; i < bmp.colors.length/4 ; i++) {
        bmp.colors[i * 4 + 4] = 255;
    }
    return bmp;
};
